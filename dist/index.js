...
(() => {
  ...
  // src/utils/dropdown-data/getPhoneData.ts
  ...
  var getPhoneData_default = fetchCountryData;

  // src/utils/dropdown-data/getUserLocation.ts
  async function getUserLocation() {
    try {
      const response = await axios_default.get("https://cloudflare.com/cdn-cgi/trace");
      const { data } = response;
  ...
  // src/utils/dropdown-comp/dropdown-controls.ts
  function focusDropdownOption(inputValue, dropdownOptions) {
    for (const option of dropdownOptions) {
      const countryCode = option.querySelector('div[data-element="value"]');
  ...
  var handleDropdownNavigation = (event, dropdownOptions) => {
    const { key } = event;
    const keyMappings = {
      ArrowUp: -1,
      ArrowDown: 1,
    ...
    const indexChange = keyMappings[key] || 0;
    if (indexChange !== 0) {
  ...
  // src/utils/dropdown-comp/populate-list.ts
  var dropdownList = document.querySelector(SELECTORS.DROPDOWN_LIST);
  ...
  function addCountryElementToPage(countryData) {
    const countryElement = createCountryElement(countryData);
    dropdownList.appendChild(countryElement);
  }

  // src/utils/dropdown-comp/dropdown.ts
  var hiddenInput = document.querySelector(SELECTORS.HIDDEN_INPUT);
  var dropdownList2 = document.querySelector(SELECTORS.DROPDOWN_LIST);
  var dropdownToggle = document.querySelector(SELECTORS.DROPDOWN_TOGGLE);
  var dropdownToggleImg = dropdownToggle.querySelector("img");
  var dropdownToggleDiv = dropdownToggle.querySelector("div");
  var currentSelectedItem = null;
  function listenToKeyPress(dropdownOptions) {
    dropdownList2.addEventListener(EVENTS.KEYDOWN, (event) => {
  ...
  function observeDropdownState() {
    new MutationObserver(() => {
      ...
      isOpen ? currentSelectedItem.focus() : "";
    }).observe(dropdownToggle, { attributes: true, attributeFilter: ["class"] });
  }
  function dropdownInit(userLocation, countryData) {
  ...
  function updateInformation(selectedCountry) {
  ...
  function selectDropdownItem(countryData) {
    ...
    for (const item of dropdownItems) {
      item.addEventListener(EVENTS.CLICK, () => {
        const selectedCountry = countryData.find(
          (country) => country.name === item.title
        );
        if (selectedCountry) {
